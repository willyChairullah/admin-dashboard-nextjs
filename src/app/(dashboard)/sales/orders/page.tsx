...
export default function OrdersPage() {
  ...
  const [orderDiscountUnit, setOrderDiscountUnit] = useState<"AMOUNT" | "PERCENTAGE">("AMOUNT");
  const [shippingCost, setShippingCost] = useState<number>(0);
  const [paymentType, setPaymentType] = useState<"IMMEDIATE" | "DEFERRED">("IMMEDIATE");
  const [paymentDeadline, setPaymentDeadline] = useState("");

  const [items, setItems] = useState<OrderItem[]>([
    { productName: "", quantity: 1, price: 0, discount: 0, discountType: "AMOUNT", crates: 0 },
  ]);
                    ...
                    <div className="min-w-0">
                      ...
                      <div className="flex gap-2">
                        ...
                        <select
                          ...
                          onChange={(e) => setOrderDiscountUnit(e.target.value as any)}
                              ...
                              <div className="flex gap-1">
                                <div className="relative flex-1">
                                  <input
                                    ...
                                    className="block w-full px-2 py-2 text-xs border-0 bg-white/90 dark:bg-gray-600/90 text-gray-900 dark:text-white placeholder-gray-500 dark:placeholder-gray-400 focus:outline-none focus:ring-2 focus:ring-amber-500 focus:border-transparent rounded-lg shadow-sm transition-all duration-200 hover:shadow-md"
                                  />
                                </div>
                                <select
                                  ...
                                  className="w-12 px-1 py-2 text-xs border-0 bg-white/90 dark:bg-gray-600/90 text-gray-900 dark:text-white focus:outline-none focus:ring-2 focus:ring-amber-500 rounded-lg shadow-sm transition-all duration-200 hover:shadow-md"
                                >
                                  <option value="AMOUNT">Rp</option>
                                  <option value="PERCENTAGE">%</option>
                                </select>
                            ...
                            <div className="w-24 sm:w-28 md:w-32 flex-shrink-0">
                              ...
                              <div className="px-3 py-3 text-sm font-bold bg-gradient-to-r from-green-100 to-emerald-100 dark:from-green-900/30 dark:to-emerald-900/30 text-green-800 dark:text-green-200 rounded-lg text-center">
                                ...
                                {(() => {
                                  ...
                                  return (itemSubtotal - itemDiscount).toLocaleString("id-ID");
                                })()}
                              </div>
                            </div>
                    ...
                    <div className="relative bg-white/80 dark:bg-gray-700/80 backdrop-blur-sm rounded-xl p-6 border border-gray-200/50 dark:border-gray-600/50">
                      ...
                      <div className="space-y-3">
                        ...
                        {calculateItemDiscounts() > 0 && (
                          <div className="flex justify-between items-center text-sm">
                            ...
                            <span className="font-bold text-red-600">
                              -Rp {calculateItemDiscounts().toLocaleString("id-ID")}
                            </span>
                          </div>
                        )}
                        {calculateOrderDiscount() > 0 && (
                          <div className="flex justify-between items-center text-sm">
                            <span className="text-gray-600 dark:text-gray-300 font-medium">
                              Diskon Order {orderDiscountUnit === "PERCENTAGE" ? `(${orderDiscount}%)` : ""}:
                            </span>
                            <span className="font-bold text-red-600">
                              -Rp {calculateOrderDiscount().toLocaleString("id-ID")}
                            </span>
                          </div>
                        )}
                        {shippingCost > 0 && (
